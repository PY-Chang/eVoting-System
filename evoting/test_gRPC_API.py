...
class TesteVotingServicer:
    ...
    #         stub.
    #         result = stub.RegisterVoter(eVoting_pb2.Voter(name="Frog", group="A", public_key=public_key_byte))
    #         assert result == 1 # voter already exists
    ...
    # def test_create_election_unknown_error(self): # code == 3
    #     with grpc.insecure_channel('localhost:50051') as channel:
    #         # create a stub for RPC call
    #         stub = eVoting_pb2_grpc.eVotingStub(channel)
    #
    ...
    #         election.groups.extend(groups)
    #         election.choices.extend(choices)
    #         election.end_date.FromDatetime("1")
    #         election.token.value = bytes(token)
    #         createElectionResponse = stub.CreateElection(election)
    #
    #         assert str(createElectionResponse.code) == '3' # unknown error
